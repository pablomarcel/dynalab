...
class GainSpec:
    ...
    def palette(self) -> PaletteInfo:
        return PaletteInfo(category="LTI", display_name="Gain", icon=None)
    ...
    def to_control(self, block: Block) -> Any:
        ...
        if errs:
            raise ValueError(f"Invalid params for gain block '{block.id}': {errs}")

        k = float(block.params["k"])
        # gain is domain-agnostic for LTI analysis; dt handled by compiler if needed.
